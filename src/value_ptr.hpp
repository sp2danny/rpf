...
namespace smart_ptr {
	...
	namespace detail {
		...
		struct ptr_base_undefined 
			...
			constexpr ptr_base_undefined( std::nullptr_t, Dx&& dx, Cx&& cx )
				: base_type(
					nullptr
					, Deleter( std::forward<Dx>( dx ), []( const void*, [[maybe_unused]] T* ptr ) { assert( ptr == nullptr ); } )
					, Copier( std::forward<Cx>( cx ), []( const void*, [[maybe_unused]] const T* ptr ) -> T* { assert( ptr == nullptr ); return nullptr; } )
				)
			{}
	...
	>
			...
			: Base
		{
			...
			template <typename Px>
			constexpr value_ptr( Px px, deleter_type dx )	// constexpr here yields c2476 on msvc15
				: value_ptr( px, std::move(dx), copier_type() )
			{}

			// construct with pointer
			template <typename Px>
			constexpr value_ptr( Px px ) // constexpr here yields c2476 on msvc15
				: value_ptr( px, deleter_type(), copier_type() )
			{}

			// std::nullptr_t, default ctor 
			explicit constexpr value_ptr( std::nullptr_t = nullptr )	// constexpr here yields c2476 on msvc15
